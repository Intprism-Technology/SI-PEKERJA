    ...
    public function gpsData(Request $request){
        try {
            if($request->start_date == $request->end_date){
                ...
                $nodes = NodesReport::where('node_id', $request->node_id)->whereBetween('created_at', [$request->start_date, $request->end_date])->where('lat', '!=', 0)->where('lng', '!=', 0)->orderBy('created_at', 'ASC')->get(['lat','lng','created_at']);
            }
            if (count($nodes) == 0) {
